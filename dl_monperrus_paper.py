...
def list_most_related_papers_to_monperrus_research():
    ...
    # Load embeddings for monperrus papers (E1: last 3 years only)
    current_year = 2026
    min_year = current_year - 3

    # Parse title -> year from monperrus.bib
    bib_path = Path.home() / "monperrus.bib"
    bib_title_to_year = {}
    current_title = None
    current_bib_year = None
    with open(bib_path, "r", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
            if line.lower().startswith("title"):
                ...
                if start != -1 and end != -1:
                    current_bib_year = int(line[start + 1 : end])
            if current_title and current_bib_year:
                bib_title_to_year[current_title] = current_bib_year
                current_title = None
                current_bib_year = None
    ...
    for paper_id in monperrus_ids:
        try:
            ...
            if data.get("paperId") == paper_id:
                ...
                year = bib_title_to_year.get(title)
    ...
    print(f"E1 (Monperrus embeddings, >= {min_year}): {len(monperrus_embeddings)}")

    # Load toread papers (E2: all papers)
    ...
    for paper_file in toread_dir.glob("*.json"):
        try:
            with open(paper_file, "r") as f:
                ...
                # print(title)
                if title:
                    embedding_data = semanticscholar_lib.get_embedding_and_push_to_db(
                        title, verbose=False
                    )
                    ...
                    if (
                        ...
                        and embedding_data["embedding"]
                        and "vector" in embedding_data["embedding"]
                        and embedding_data["embedding"]["vector"]
                    ):
                        ...
                        if len(vector) > 0:
                            ...
                            if (
                                not tldr
                                and "tldr" in embedding_data
                                and embedding_data["tldr"]
                            ):
    ...
    print(f"E2 (toread embeddings, all years): {len(toread_papers)}")
    ...
    for toread_paper in toread_papers:
        ...
        for paper_id, monperrus_data in monperrus_embeddings.items():
            similarity = cosine_similarity(
                toread_paper["vector"], monperrus_data["vector"]
            )
            ...
            results.append(
                {
                    ...
                    "angle": np.arccos(1 - min_distance)
                    * (180 / np.pi),  # Convert distance to angle in degrees
