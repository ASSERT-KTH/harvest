...
if __name__ == "__main__":
    ...
    for url in urls:
        ...
        try:
            ...
            if data and "doi" in data and data["doi"] and len(data["doi"])>0:
                paperId = semanticscholar_lib.get_semantic_scholar_id_from_url(data["doi"], prefix="DOI:")
            elif "arxiv.org" in url:
                paperId = semanticscholar_lib.get_semantic_scholar_id_from_url(url, prefix="URL:")
            elif data and "semanticscholarid" in data and len(data["semanticscholarid"])>0 and "arxiv.org" not in url:
                paperId = data["semanticscholarid"] 
            elif data and "title" in data and len(data["title"])>0:
                ssdata = semanticscholar_lib.get_semantic_scholar_id_from_title(data["title"])
                if ssdata:
    ...
    for paperId in paperIds:
        ...
        if paper_data and "embedding" in paper_data and 'title' in paper_data and paper_data["embedding"] and "vector" in paper_data["embedding"]:
    ...
    for paperId, paper_data in paper_embeddings.items():
        if paper_data and "embedding" in paper_data and paper_data["embedding"] and "vector" in paper_data["embedding"]:
            ...
            try:
                results = embed.search_in_pinecone_semanticscholar(paper_data.get("title", "query"), vector, top_k=10)
                for res in results:
                    title = res["title"]
