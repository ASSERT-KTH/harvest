...
citing_rx = re.compile(
    "^(.*) [-–] (new citations|nya citat|de nouvelles citations|neue Zitationen)"
)  # – nya citat
...
def extract_my_articles(data_txt):
    ...
    for i in data.xpath('.//a[@class="gse_alrt_title"]'):
        ...
        paper.reason.append(
            "citing: "
            + i.xpath("../following-sibling::table")[0].xpath(".//span/text()")[0]
        )
...
class ScholarScraper:
    ...
    def feed(self, message):
        data = base64.urlsafe_b64decode(str(message["payload"]["body"]["data"])).decode(
            "utf-8"
        )
...
def already_seen(paper):
    ...
    if paper.desc.lower() in READING_NOTES:
        return True
...
def get_zotero_translator_service_url(url):
    ...
    """
    fname = path_on_disk_internal_v2(
        url,
        "/home/martin/workspace/scholar-harvest/cache/get_zotero_translator_service/",
    )
    if os.path.exists(fname):
        with open(fname, "r") as f:
            try:
                ...
                return None
    resp = requests.post(
        "https://t0guvf0w17.execute-api.us-east-1.amazonaws.com/Prod/web",
        data=url,
    ...
    if (
        ...
        or '"message"' in resp.text
        or "No items returned from any translator" in resp.text
    ):
        result = []
    else:
        try:
            ...
            if "creators" in result[i]:
                for o in range(len(result[i]["creators"])):
                    if "name" in result[i]["creators"][o]:
                        result[i]["creators"][o]["lastName"] = result[i]["creators"][o][
                            "name"
                        ]
...
def get_cdsl_data(csdlid):
    ...
    if (
        "article" in response["data"]
        and response["data"]["article"]
        and "doi" in response["data"]["article"]
    ):
...
def create_harvest_email_paper(paper, service, **kwargs):
    ...
    if "detection_date" in kwargs:
        detection_date = kwargs["detection_date"]
    if already_seen(paper):
        return False

    paper_data = collect_paper_data_from_url_with_cache(
        paper.url, reason=paper.print_reason()
    )
    ...
    try:
        ...
        if (
            semanticscholar
            and semanticscholar["embedding"]
            and semanticscholar["embedding"]["vector"]
        ):
            paper_data["note"] = "related in embedding space:\n- " + "\n- ".join(
                [
                    x["title"]
                    for x in rrs.search_in_pinecone_semanticscholar(
                        paper_data["title"], semanticscholar["embedding"]["vector"], 5
                    )
                ]
            )
            try:
                paper_data["tldr"] = semanticscholar["tldr"]["text"]
            except:
                pass
    ...
    if is_high_reputation(paper.url):
        fname = path_on_disk_internal_v2(
            paper.desc, "/home/martin/workspace/scholar-harvest/cache/toread/"
        )
...
def is_high_reputation(url):
    ...
    if "onlinelibrary.wiley.com" in url:
        return True  # metadata via zotero, via crossref
    ...
    if "ojs.aaai.org" in url:
        return True
    if "bitstream" in url:
        return True  # dspace, probably from a university repository
    if "handle.net" in url:
        return True
    if "openreview.net" in url:
        return True
    if "google.com" in url:
        return True  # for patents.google.com mostly
    return False
...
def collect_paper_data_from_url_with_cache(url, reason=None):
    urlseen, thepath = already_seen_url(
        url, "/home/martin/workspace/scholar-harvest/cache/harvest/"
    )
    if urlseen:
    ...
    if data:
        ...
        if data["title"] and len(data["title"]) > 0:
            ...
            # double linking
            _, titlepath = already_seen_url(
                data["title"], "/home/martin/workspace/scholar-harvest/cache/harvest/"
            )
...
def info_from_crossref(doi):
    ...
    year = (
        message.get("published", {}).get("date-parts", [[]])[0][0]
        if message.get("published")
        else ""
    )
    ...
    if resp.status_code == 200:
        ...
        if meta_doi:
            ...
            for item in meta_dc_id:
                if (
                    "doi.org" in item and len(item) > 20
                ):  # simple check to avoid empty http://dx.doi.org/
                    doi = item.replace("http://dx.doi.org/", "").replace(
                        "https://doi.org/", ""
                    )
...
def collect_paper_data_from_arxiv(url):
    try:
        ...
        venue_title = (
            arxiv_metadata.get("journal_ref")
            if arxiv_metadata.get("journal_ref")
            and len(arxiv_metadata.get("journal_ref")) > 0
            else "arXiv"
        )
...
def collect_paper_data_from_diva(url):
        ...
        if person.get("role", {}).get("roleTerm") == "aut":
            ...
            if len(name_parts) >= 2:
                ...
                affiliation = (
                    ...
                    if type(person.get("affiliation", "")) == str
                    else person.get("affiliation", [""])[0]
                )
    ...
    try:
        abstract_html = (
            mods.get("abstract")
            if type(mods.get("abstract")) == str
            else mods.get("abstract", ["no abstract"])[0]
        )
    except:
        pass
    ...
    try:
        ...
        if "link.springer.com" in url:
            ...
            if "records" in springer_data and len(springer_data["records"]) > 0:
                ...
                print("no records found in Springer " + url)
        ...
        if "sciencedirect.com" in url:
            ...
            if match:
                ...
                elsevier_data = requests.get(
                    api_url, headers={"x-els-apikey": config.sciencedirect_key}
                ).json()
                # print("elsevier_data",pii,json.dumps(elsevier_data, indent=2))
                if "full-text-retrieval-response" in elsevier_data:
                    venue_title = elsevier_data["full-text-retrieval-response"][
                        "coredata"
                    ]["prism:publicationName"]
                    doi = elsevier_data["full-text-retrieval-response"]["coredata"][
                        "prism:doi"
                    ]
                    title = elsevier_data["full-text-retrieval-response"]["coredata"][
                        "dc:title"
                    ]
                    abstract = elsevier_data["full-text-retrieval-response"][
                        "coredata"
                    ]["dc:description"]
                    url = url.replace("?dgcid=rss_sd_all", "")
                    author_list = []
                    for x in elsevier_data["full-text-retrieval-response"]["coredata"][
                        "dc:creator"
                    ]:
        ...
        if "ieeexplore.ieee.org" in url:
            ...
            if "abs_all.jsp" in url:
                ...
                ieeeid = [x for x in url.split("/") if len(x) > 0][-1].replace(
                    ".pdf", ""
                )
            # curl "https://ieeexploreapi.ieee.org/api/v1/search/articles?apiKey=XXXXXX&article_number=10833642"

            req_url = (
                "https://ieeexploreapi.ieee.org/api/v1/search/articles?article_number="
                + ieeeid
                + "&apiKey="
                + config.ieeexplore_key
            )
            ...
            if resp.status_code != 418:  # i'm a teapot, WTF?
                ...
                try:
                    ...
                    return None
                    raise Exception(
                        "ieee error", resp.status_code, resp.text, resp.headers
                    )
                ...
                if "error" not in ieeedata and "articles" in ieeedata:
                    ...
                    abstract = (
                        ieeedata["articles"][0]["abstract"]
                        if "abstract" in ieeedata["articles"][0]
                        else ""
                    )
                    ...
                    author_list = [
                        x["full_name"]
                        for x in ieeedata["articles"][0]["authors"]["authors"]
                    ]
                    authors = ", ".join(
                        [
                            x["full_name"]
                    ...
                    print("no data in ieee " + json.dumps(ieeedata, indent=2))
    ...
    if len(path_parts) >= 3 and path_parts[-2].isdigit() and path_parts[-1].isdigit():
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                metadata = xml_root.find(
                    ".//oai:record/oai:metadata/oai_dc:dc", dc_namespace
                )
                if metadata is not None:
                    ...
                    authors = (
                        ", ".join([author.text for author in author_elements])
                        if author_elements
                        else ""
                    )
                    ...
                    for identifier in identifiers:
                        if identifier.text and "doi" in identifier.text:
                            doi = identifier.text.replace("doi:", "").replace(
                                "https://dx.doi.org/", ""
                            )
                            break
...
def collect_paper_data_from_preprints_org(url):
    ...
    try:
        ...
        # or: https://www.preprints.org/frontend/manuscript/HASH/download_pub
        manuscript_pattern = r"/manuscript/([0-9]+\.[0-9]+)"
        hash_pattern = r"/manuscript/([a-f0-9]{32})(?:/|$)"
        match = re.search(manuscript_pattern, url)
        hash_match = re.search(hash_pattern, url)

        if not match and hash_match:
            # Hash-based URL (e.g. /frontend/manuscript/HASH/download_pub)
            # The page at this URL returns 403 even with browser headers.
            # Instead, download the PDF and extract the preprints DOI from it,
            # then look up metadata via CrossRef.
            manuscript_id = hash_match.group(1)
            note = f"preprints.org manuscript: {manuscript_id}"
            try:
                cmd = ["curl-cffi-cli.py", url]
                result = subprocess.run(cmd, capture_output=True, timeout=30)
                pdf_bytes = result.stdout
                # Extract preprints DOI from PDF binary (e.g. 10.20944/preprints202603.0295.v1)
                doi_match = re.search(
                    rb"10\.20944/preprints(\d{6}\.\d+\.v\d+)", pdf_bytes
                )
                if doi_match:
                    doi = "10.20944/preprints" + doi_match.group(1).decode()
                    crossref_data = info_from_crossref(doi)
                    if crossref_data and crossref_data.get("title"):
                        ...
                        venue_title = crossref_data.get("venue_title", "Preprints.org")
                        year = crossref_data.get("year", year)
                else:
                    print(f"Could not extract DOI from Preprints.org PDF: {url}")
            except Exception as e:
                print(f"Error downloading/parsing Preprints.org PDF from {url}: {e}")
        ...
        if match:
            ...
            if response.status_code == 200:
                ...
                abstract_meta = html_root.xpath(
                    '//meta[@name="citation_abstract"]/@content'
                )
                if abstract_meta:
                    abstract = abstract_meta[0]
                elif html_root.xpath('//meta[@name="description"]/@content'):
                    abstract = html_root.xpath('//meta[@name="description"]/@content')[
                        0
                    ]
...
def collect_paper_data_from_openreview(url):
    ...
    if content.get("authors"):
        authors_list = (
            content.get("authors").get("value") or content.get("authorids") or []
        )
    else:
...
def collect_paper_data_from_semanticscholar(url):
    ...
    tldr = (
        semanticscholar["tldr"]["text"] + "\n\n"
        if "tldr" in semanticscholar
        and semanticscholar["tldr"]
        and semanticscholar["tldr"]["text"]
        else ""
    )
    authors = ""
...
def old_code_for_tldr():
    ...
    if semanticscholarid != "" and (
        ...
        semanticscholar = requests.get(
            "https://api.semanticscholar.org/graph/v1/paper/"
            + semanticscholarid
            + "?fields=title,tldr,authors,embedding,embedding.specter_v2",
        ...
        if (
            ...
            and "tldr" in semanticscholar
            and semanticscholar["tldr"] != None
            and semanticscholar["tldr"]["text"]
        ):
        ...
        if (
            semanticscholar != None
            and "embedding" in semanticscholar
            and semanticscholar["embedding"]
            and "vector" in semanticscholar["embedding"]
        ):
            note = "related in embedding space:\n- " + "\n- ".join(
                [
                    x["title"]
                    for x in rrs.search_in_pinecone_semanticscholar(
                        title, semanticscholar["embedding"]["vector"], 6
                    )
                    if x["title"] != title
                ]
            )
...
def notify_email(paper, service):
    ...
    """
    path = path_on_disk_internal_v2(
        paper.desc, "/home/martin/workspace/scholar-harvest/cache/already_notified/"
    )
    ...
    encoded_title = (
        "=?utf-8?B?"
        + base64.b64encode(paper.desc.encode("utf-8")).decode("ascii")
        + "?="
    )

    # must end with \n\n
    header = (
        "From: <harvest@monperrus.net>\nTo: <martin.monperrus@gmail.com>\nSubject: "
        + encoded_title
        + "\n\n"
    )
...
<body>{email_html_body}
    ...
    if (
        service != None
        and str(type(service)) == "<class 'googleapiclient.discovery.Resource'>"
    ):
...
def push_email_via_gmail(service, msg, category_label_ids):
    ...
    try:
        ...
        print(
            "error when getting message id, should probably sleep but this would become too slow",
            e,
        )
        rfc822msgid = None
...
def send_email(encoded_title, email, recipients):
    ...
    args.sender_password = sendemail.login_keyring.get_password(
        "login2", args.sender_email
    )
    args.receiver_email = ""
...
def get_labelId(category):
    ...
    if category == "planetse":
        return "Label_816000980291680327"
                ...
                for paper_id, entry in data.items():
                    ...
                    if "metadata" in entry and "title" in entry["metadata"]:
                        ...
                        if main_title:
                            normalized = normalize_title(
                                main_title.replace("\\\\", "").strip().lower()
                            )
                    ...
                    if "citations" in entry:
                        for cited_title in entry.get("citations", []):
                            if cited_title:
                                normalized = normalize_title(
                                    cited_title.replace("\\\\", "").strip().lower()
                                )
                                if normalized not in _overleaf_mapping_cache:
                                    _overleaf_mapping_cache[normalized] = []
                                if (
                                    main_title
                                    and main_title
                                    not in _overleaf_mapping_cache[normalized]
                                ):
                                    _overleaf_mapping_cache[normalized].append(
                                        main_title
                                    )
            except Exception as e:
...
def compute_categories_embedding_test2():
    ...
    for path in paths:
        ...
        if (
            ...
            or not embedding_result.get("embedding")
            or not embedding_result["embedding"].get("vector")
        ):
            continue
...
def compute_category_based_past_classification(paper):
    ...
    if title_lower in _topic_mapping_cache:
        return [
            (
                "compute_category_based_past_classification",
                _topic_mapping_cache[title_lower],
            )
        ]
...
def classify_scholarnotifications():
    ...
    response = (
        scraper.service.users()
        .messages()
        .list(userId="me", q=query, maxResults=maxRes)
        .execute()
    )
    ...
    for m in messages:
        ...
        scraper.service.users().messages().modify(
...
def classify_planetse():
    ...
    for m in messages:
        ...
        if (
            embedding
            and embedding.get("embedding")
            and embedding["embedding"].get("vector")
        ):
        ...
        if classification != "uncategorized":
            ...
            create_harvest_email_paper(
                paper, service, origin="planetse", detection_date=msg_date
            )
    ...
    for m in messages:
        ...
        for payload in msg["payload"]["parts"]:
            ...
            for i in doc.xpath(".//a[contains(@class,'paper-link')]"):
                ...
                if create_harvest_email_paper(
                    paper, service, origin="semanticscholar", detection_date=msg_date
                ):
...
def transfer_data_from_dict_to_paper(paper, paper_data):
    paper.url = (
        paper_data["url"]
        if "url" in paper_data and paper_data["url"] and len(paper_data["url"]) > 0
        else paper.url
    )
    # OOPS for historical reasons paper.desc is title
    paper.desc = (
        paper_data["title"]
        if "title" in paper_data and len(paper_data["title"]) > 0
        else paper.desc
    )
    if "author_list" in paper_data and paper_data["author_list"]:
        paper.authors = (
            " | ".join(paper_data["author_list"])
            if "author_list" in paper_data and paper_data["author_list"]
            else ""
        )
...
def notify_10_last_author_alerts():
    ...
    for filepath in harvest_files:
        try:
            ...
            timestamp_value = paper_data.get("timestamp") or paper_data.get(
                "detection_date"
            )
            if timestamp_value:
                try:
...
def notify_for_all_categorized_papersto_read():
    ...
    toread_dir = "/home/martin/workspace/scholar-harvest/cache/toread/"
    already_notified_dir = (
        "/home/martin/workspace/scholar-harvest/cache/already_notified/"
    )
    ...
    for filepath in toread_files:
        try:
            ...
            if "reason" in paper_data:
                paper.reason = (
                    paper_data["reason"].split(", ")
                    if isinstance(paper_data["reason"], str)
                    else paper_data["reason"]
                )

            # Classify the paper
            paper.categories = paper_data.get("categories", [])
            paper.category = (
                paper.categories[0] if paper.categories else "uncategorized"
            )
            ...
            if paper.category != "uncategorized" and len(paper.categories) > 0:
                ...
                notified_path = path_on_disk_internal_v2(
                    paper.desc, already_notified_dir
                )
...
def backtrack_to_get_missing_embeddings():
    ...
    harvest_files = glob.glob(
        "/home/martin/workspace/scholar-harvest/cache/harvest/*.json"
    )
        ...
        try:
            ...
            if embedding_result and embedding_result.get("embedding"):
                ...
                if withembedding % 10 == 0:
                    print(
                        f"Processed {withembedding} papers with embeddings, {missingembedding} missing..."
                    )
            else:
                missingembedding += 1
...
def main():
    ...
    READING_NOTES = (
        open(
            "/home/martin/workspace/related-work-github/ASSERT-KTH-related-work/allall.md"
        )
        .read()
        .lower()
    )
