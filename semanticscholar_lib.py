...
def get_embedding_and_push_to_db(
    title,
    output_dir="/home/martin/workspace/scholar-harvest/cache/embedding.specter_v2/",
    verbose=False,
    delay=1.0,
):
    ...
    if (
        ...
        and "vector" in result["embedding"]
    ):
...
def get_embedding(
    ...
    if os.path.exists(target_path):
        ...
        if (
            not data
            or not "embedding" in data
            or not data["embedding"]
            or "vector" not in data["embedding"]
        ):
    ...
    try:
        url = (
            "https://api.semanticscholar.org/graph/v1/paper/search/match?query=" + title
        )
        semanticscholar = requests.get(
            url, headers={"x-api-key": config.semanticscholar_key}
        )
        ...
        if "data" not in resp or not resp["data"]:
            ...
            with open(not_found_path, "w") as f:
                f.write(
                    json.dumps(
                        {"url": url, "title": title, "response": semanticscholar.text},
                        indent=2,
                    )
                )
            return None
        ...
        if (
            not semanticscholarfull
            or "status" in semanticscholarfull
            and semanticscholarfull["status"] == "not found"
        ):
            with open(not_found_path, "w") as f:
                f.write(
                    json.dumps(
...
def get_embedding_from_paper_id(semanticscholarid, delay=SEMANTICSCHOLAR_DELAY):
    ...
    """
    cache_dir = (
        "/home/martin/workspace/scholar-harvest/cache/get_embedding_from_paper_id/"
    )
    ...
    if os.path.exists(not_found_path):
        if (
            time.time() - os.path.getmtime(not_found_path)
            > MAX_AGE_BEFORE_RETRY_IN_DAYS * 24 * 60 * 60
        ):
    ...
    if "authors" in semanticscholarfull and isinstance(
        semanticscholarfull["authors"], list
    ):
        semanticscholarfull["authors_list"] = semanticscholarfull["authors"]
        semanticscholarfull["authors"] = (
            ", ".join([a["name"] for a in semanticscholarfull["authors"]])
            if "authors" in semanticscholarfull
            else ""
        )

    # raise Exception(semanticscholarfull)
    if (
        "embedding" not in semanticscholarfull
        or not semanticscholarfull["embedding"]
        or "vector" not in semanticscholarfull["embedding"]
    ):
        data = {
            "url": url,
            "status": "not found",
...
def get_semantic_scholar_id_from_title(title):
    ...
    fname = path_on_disk_internal_v2(
        title,
        "/home/martin/workspace/scholar-harvest/cache/get_semantic_scholar_id_from_title/",
    )
    if os.path.exists(fname):
        with open(fname, "r") as f:
            try:
    ...
    if "data" in orig_data:
        ...
        raise Exception("not found in SemanticScholar")
    with open(fname, "w") as f:
        json.dump(data, f)
    return data
...
def get_semantic_scholar_id_from_url(url, prefix="URL:"):
    ...
    if os.path.exists(fname):
        with open(fname, "r") as f:
            ...
            return data["paperId"]
    semanticscholar = requests.get(
        f"https://api.semanticscholar.org/graph/v1/paper/{prefix}" + url,
        headers={"x-api-key": config.semanticscholar_key},
    )
    time.sleep(SEMANTICSCHOLAR_DELAY)
...
def get_url_from_title(title):
    ...
    if normalize_title(result["title"]) == normalize_title(title):
        ...
        if "externalIds" in data:
            ...
            raise Exception(
                "No DOI or ArXiv found for title: " + str(data["externalIds"])
            )
...
def get_recommended_papers(
    paper_id,
    cache_dir="/home/martin/workspace/scholar-harvest/cache/recommendations/",
    verbose=False,
):
    """
...
def get_citing_papers(paper_id, verbose=False):
    ...
    if os.path.exists(cache_file):
        ...
        print(
            f"Loading cached citing papers for paper ID: {paper_id} (cached {cache_age_days:.1f} days ago)"
        )
    ...
    if verbose:
        print(
            f"Fetching citing papers from SemanticScholar API for paper ID: {paper_id}"
        )
    ...
    data = data["data"]
    data = [
        d["citingPaper"]["paperId"]
        for d in data
        if "citingPaper" in d
        and "paperId" in d["citingPaper"]
        and d["citingPaper"]["paperId"]
    ]
...
def get_cited_papers(
    paper_id,
    cache_dir="/home/martin/workspace/scholar-harvest/cache/cited_papers/",
    verbose=True,
):
    """
    ...
    data = [
        ...
        for d in data
        if "citedPaper" in d
        and "paperId" in d["citedPaper"]
        and d["citedPaper"]["paperId"]
    ]
